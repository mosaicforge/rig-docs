...
Often times if you just want to create simple tools and don't need any special particular implementation details, writing out the entire `impl Tool` block can be quite cumbersome.

Fortunately, the `rig_derive` crate has a helpful derive macro called `tool_macro` which you can use to create your own tools easily. See below for an example of usage:
